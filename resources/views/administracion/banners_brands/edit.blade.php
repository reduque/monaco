...
<form role="form" action="{{ route('banners_brands.update', codifica($banner->id)) }}" method="POST">
    ...
    <div class="row">
        <div class="col-lg-6">
            ...
            <a href="{{ route('banners_brands.create') }}" class="btn btn-primary"><i class="fa fa-fw fa-plus-circle"></i> @lang('administracion.nuevo')</a> 
            <a href="{{ route('banners_brands_eliminar', codifica($banner->id) ) }}" class="btn btn-danger"><i class="fa fa-fw fa-ban"></i> @lang('administracion.eliminar')</a>
        </div>
    </div>
</form>
